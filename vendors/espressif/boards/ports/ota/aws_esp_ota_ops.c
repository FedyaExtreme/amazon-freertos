...
static const esp_partition_t *_esp_get_otadata_partition(uint32_t *offset,
  ...
  find_partition = esp_partition_find_first(
      ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_DATA_OTA, NULL);
  if (find_partition != NULL) {
    ret = esp_partition_mmap(find_partition, 0, find_partition->size,
                             SPI_FLASH_MMAP_DATA, &result, &ota_data_map);
    if (ret != ESP_OK) {
    ...
    uint32_t gen_0_seq =
        ota_select_valid(&s_ota_select[0]) ? s_ota_select[0].ota_seq : 0;
    uint32_t gen_1_seq =
        ota_select_valid(&s_ota_select[1]) ? s_ota_select[1].ota_seq : 0;
    if (gen_0_seq == 0 && gen_1_seq == 0) {
      ESP_LOGW(TAG,
               "otadata partition is invalid, factory/ota_0 is boot partition");
      memcpy(entry, &s_ota_select[0], sizeof(ota_select));
      *offset = 0;
    } else if ((gen_0_seq >= gen_1_seq && active_part) ||
               (gen_1_seq > gen_0_seq && !active_part)) {
      memcpy(entry, &s_ota_select[0], sizeof(ota_select));
      *offset = 0;
      ESP_LOGI(TAG, "[0] aflags/seq:0x%x/0x%x, pflags/seq:0x%x/0x%x",
               s_ota_select[0].ota_state, gen_0_seq, s_ota_select[1].ota_state,
               gen_1_seq);
    } else {
      ...
      *offset = SPI_FLASH_SEC_SIZE;
      ESP_LOGI(TAG, "[1] aflags/seq:0x%x/0x%x, pflags/seq:0x%x/0x%x",
               s_ota_select[1].ota_state, gen_1_seq, s_ota_select[0].ota_state,
               gen_0_seq);
    }
  } else {
...
esp_err_t aws_esp_ota_set_boot_flags(uint32_t flags, bool active_part) {
  ...
  ESP_LOGD(TAG, "%s: %d %d", __func__, flags, active_part);
...
esp_err_t aws_esp_ota_get_boot_flags(uint32_t *flags, bool active_part) {
  ...
  ESP_LOGD(TAG, "%s: %d", __func__, active_part);
