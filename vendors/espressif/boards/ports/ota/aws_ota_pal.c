...
#define kOTA_HalfSecondDelay pdMS_TO_TICKS(4000UL)
...
/* Check configuration for memory constraints provided SPIRAM is not enabled */
    ...
    (configENABLED_NETWORKS & AWSIOT_NETWORK_TYPE_BLE)
#error                                                                         \
    "Cannot enable OTA data over HTTP together with BLE because of not enough heap."
#endif
...
static CK_RV prvGetCertificate(const char *pcLabelName, uint8_t **ppucData,
                               uint32_t *pulDataSize);

static OTA_Err_t asn1_to_raw_ecdsa(uint8_t *signature, uint16_t sig_len,
  ...
  ret = mbedtls_mpi_write_binary(&s, out_signature + ECDSA_INTEGER_LEN,
                                 ECDSA_INTEGER_LEN);
...
static bool _esp_ota_ctx_validate(OTA_FileContext_t *C) {
  return (C != NULL && ota_ctx.cur_ota == C &&
...
static CK_RV prvGetCertificateHandle(CK_FUNCTION_LIST_PTR pxFunctionList,
  ...
  if (0 == xResult) {
    ...
    xResult = pxFunctionList->C_FindObjects(
        xSession, (CK_OBJECT_HANDLE_PTR)pxCertHandle, 1, &ulCount);
  }
...
static CK_RV prvGetCertificate(const char *pcLabelName, uint8_t **ppucData,
  ...
  if (CKR_OK == xResult) {
    xResult = xFunctionList->C_OpenSession(xSlotId, CKF_SERIAL_SESSION, NULL,
                                           NULL, &xSession);
  }
  ...
  if (CKR_OK == xResult) {
    ...
    xResult =
        prvGetCertificateHandle(xFunctionList, xSession, pcLabelName, &xHandle);
  }
  ...
  if ((xHandle != 0) && (xResult == CKR_OK)) /* 0 is an invalid handle */
  {
    ...
    xResult = xFunctionList->C_GetAttributeValue(xSession, xHandle, &xTemplate,
                                                 xCount);
    ...
    if (xResult == CKR_OK) {
      ...
      xResult = xFunctionList->C_GetAttributeValue(xSession, xHandle,
                                                   &xTemplate, xCount);
...
u8 *prvPAL_ReadAndAssumeCertificate(const u8 *const pucCertName,
  ...
  xResult = prvGetCertificate((const char *)pucCertName, &pucSignerCert,
                              ulSignerCertSize);
  ...
  if ((xResult == CKR_OK) && (pucSignerCert != NULL)) {
    ...
    pucSignerCert = pvPortMalloc(
        ulCertSize); /*lint !e9029 !e9079 !e838 malloc proto requires void*. */
    pucCertData = (uint8_t *)
        signingcredentialSIGNING_CERTIFICATE_PEM; /*lint !e9005 we don't modify
                                                     the cert but it could be
                                                     set by PKCS11 so it's not
                                                     const. */
    ...
    if (pucSignerCert != NULL) {
      ...
      *ulSignerCertSize = ulCertSize;
      ...
      ESP_LOGE(TAG, "Error: No memory for certificate in "
                    "prvPAL_ReadAndAssumeCertificate!\r\n");
    }
  }
...
OTA_Err_t prvPAL_CheckFileSignature(OTA_FileContext_t *const C) {
  ...
  if (CRYPTO_SignatureVerificationStart(
  ...
  pucSignerCert = prvPAL_ReadAndAssumeCertificate(
      (const u8 *const)C->pucCertFilepath, &ulSignerCertSize);
  ...
  esp_err_t ret =
      esp_partition_mmap(ota_ctx.update_partition, 0, ota_ctx.data_write_len,
  ...
  CRYPTO_SignatureVerificationUpdate(pvSigVerifyContext, buf,
                                     ota_ctx.data_write_len);
  spi_flash_munmap(ota_data_map);

  if (CRYPTO_SignatureVerificationFinal(
          pvSigVerifyContext, (char *)pucSignerCert, ulSignerCertSize,
...
OTA_Err_t prvPAL_CloseFile(OTA_FileContext_t *const C) {
  ...
  if (C->pxSignature == NULL) {
    ...
    /* Verify the file signature, close the file and return the signature
     * verification result. */
    ...
    if (result != kOTA_Err_None) {
      ...
      /* Write ASN1 decoded signature at the end of firmware image for
       * bootloader to validate during bootup */
      esp_sec_boot_sig_t *sec_boot_sig =
          (esp_sec_boot_sig_t *)malloc(sizeof(esp_sec_boot_sig_t));
      ...
      if (sec_boot_sig != NULL) {
        ...
        result =
            asn1_to_raw_ecdsa(C->pxSignature->ucData, C->pxSignature->usSize,
                              sec_boot_sig->raw_ecdsa_sig);

        if (result == kOTA_Err_None) {
          esp_err_t ret =
              esp_ota_write_with_offset(ota_ctx.update_handle, sec_boot_sig,
                                        ECDSA_SIG_SIZE, ota_ctx.data_write_len);
...
int16_t prvPAL_WriteBlock(OTA_FileContext_t *const C, uint32_t iOffset,
                          uint8_t *const pacData, uint32_t iBlockSize) {
  if (_esp_ota_ctx_validate(C)) {
    esp_err_t ret = esp_ota_write_with_offset(ota_ctx.update_handle, pacData,
                                              iBlockSize, iOffset);
...
OTA_PAL_ImageState_t prvPAL_GetPlatformImageState() {
  ...
  if ((ota_ctx.cur_ota != NULL) && (ota_ctx.data_write_len != 0)) {
    ...
    ota_flags =
        ota_ctx.valid_image == true ? ESP_OTA_IMG_NEW : ESP_OTA_IMG_INVALID;
  } else {
...
OTA_Err_t prvPAL_SetPlatformImageState(OTA_ImageState_t eState) {
  ...
  /* If this is first request to set platform state, post bootup and there is
   * not OTA being triggered yet, then operate on active image flags, else use
   * passive image flags */
  if ((ota_ctx.cur_ota == NULL) && (ota_ctx.data_write_len == 0)) {
    if (ota_flags == ESP_OTA_IMG_PENDING_VERIFY) {
      ...
      eResult = ota_flags == ESP_OTA_IMG_VALID ? kOTA_Err_None
