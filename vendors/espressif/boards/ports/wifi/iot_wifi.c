...
static void event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT) {
    ...
    case SYSTEM_EVENT_STA_DISCONNECTED:
      ESP_LOGI(TAG, "SYSTEM_EVENT_STA_DISCONNECTED: %d",
      ...
      if (xWifiEventHandlers[eWiFiEventDisconnected] != NULL) {
        ...
        eventInfo.xInfo.xDisconnected.xReason =
            convertReasonCode(info->disconnected.reason);
    ...
    case IP_EVENT_STA_GOT_IP:
      ...
      if (xWifiEventHandlers[eWiFiEventIPReady] != NULL) {
        ...
        eventInfo.xInfo.xIPReady.xIPAddress.ulAddress[0] =
...
static void sc_callback(void *arg, esp_event_base_t event_base,
                        int32_t event_id, void *event_data) {
  switch (event_id) {
...
WIFIReturnCode_t WIFI_Provision() {
  ...
  if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
    ...
    xEventGroupWaitBits(wifi_event_group, STARTED_BIT, pdFALSE, pdFALSE,
                        portMAX_DELAY);
    ...
    xEventGroupWaitBits(wifi_event_group, ESPTOUCH_DONE_BIT | DISCONNECTED_BIT,
  ...
  if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
    if ((wifi_conn_state == true) &&
        ((pxNetworkParams == NULL) ||
         (memcmp(pxNetworkParams->ucSSID, xConnectedAP.ucSSID,
  ...
  if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
    if (wifi_conn_state == true) {
      ...
      if (ret == ESP_OK) {
        ...
        xEventGroupWaitBits(wifi_event_group, DISCONNECTED_BIT, pdTRUE, pdFALSE,
                            portMAX_DELAY);
      } else {
...
#define CHECK_VALID_SSID_LEN(x) ((x) > 0 && (x) <= wificonfigMAX_SSID_LEN)
...
WIFI_ConnectAP(const WIFINetworkParams_t *const pxNetworkParams) {
  ...
  if (pxNetworkParams->xSecurity != eWiFiSecurityOpen &&
      !CHECK_VALID_PASSPHRASE_LEN(pxNetworkParams->xPassword.xWPA.ucLength)) {
    return wifi_ret;
  }
  ...
  if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
    ...
    // Wait for wifi connected or disconnected event
    xEventGroupWaitBits(wifi_event_group, CONNECTED_BIT | DISCONNECTED_BIT,
                        pdTRUE, pdFALSE, portMAX_DELAY);
    if (wifi_conn_state == true) {
...
WIFIReturnCode_t WIFI_Reset(void) { return eWiFiNotSupported; }
/*-----------------------------------------------------------*/
  ...
  if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
    ...
    if (ret == ESP_OK) {
      wifi_ap_record_t *ap_info =
          calloc(1, sizeof(wifi_ap_record_t) * ucNumNetworks);
      if (ap_info != NULL) {
        uint16_t num_aps = ucNumNetworks;
        esp_wifi_scan_get_ap_records(&num_aps, ap_info);
        for (int i = 0; i < num_aps; i++) {
          pxBuffer[i].ucSSIDLength =
              strlcpy((char *)pxBuffer[i].ucSSID, (const char *)ap_info[i].ssid,
                      wificonfigMAX_SSID_LEN + 1);
          memcpy(pxBuffer[i].ucBSSID, ap_info[i].bssid,
        ...
        ESP_LOGE(TAG, "%s: Failed to alloc %lu bytes for WIFI provisionning",
                 __func__,
                 (long unsigned int)(sizeof(wifi_ap_record_t) * ucNumNetworks));
        ESP_LOGE(TAG, "%s: Current heap: %lu, min heap %lu", __func__,
                 (long unsigned int)xPortGetFreeHeapSize(),
                 (long unsigned int)xPortGetMinimumEverFreeHeapSize());
      }
    }
...
esp_err_t
prxSerializeWifiNetworkProfile(const WIFINetworkProfile_t *const pxNetwork,
                               uint8_t *pucBuffer, uint32_t *pulSize) {
...
esp_err_t
prxDeSerializeWifiNetworkProfile(WIFINetworkProfile_t *const pxNetwork,
                                 uint8_t *pucBuffer, uint32_t ulSize) {
  esp_err_t xRet = ESP_OK;
  uint32_t ulLen;
...
esp_err_t prvGetWIFINetwork(nvs_handle xNvsHandle,
                            WIFINetworkProfile_t *pxNetwork, uint16_t usIndex) {
  uint32_t ulSize = 0;
...
WIFI_NetworkAdd(const WIFINetworkProfile_t *const pxNetworkProfile,
  ...
  if (pxNetworkProfile != NULL && pusIndex != NULL) {
    ...
    if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
      ...
      if (xRet == ESP_OK) {
        xRet = prvStoreWIFINetwork(xNvsHandle, pxNetworkProfile,
                                   xRegistry.usNextStorageIdx);
      }

      if (xRet == ESP_OK) {

        xRegistry.usStorageIdx[xRegistry.usNumNetworks] =
            xRegistry.usNextStorageIdx;
        xRegistry.usNextStorageIdx++;
        xRegistry.usNumNetworks++;
        xRet =
            nvs_set_blob(xNvsHandle, "registry", &xRegistry, sizeof(xRegistry));
      }
    ...
    if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
      ...
      if (xRet == ESP_OK) {
        if (usIndex < xRegistry.usNumNetworks) {
          xRet = prvGetWIFINetwork(xNvsHandle, pxNetworkProfile,
                                   xRegistry.usStorageIdx[usIndex]);
        } else {
          xRet = ESP_FAIL;
        }
      }
...
WIFIReturnCode_t WIFI_NetworkDelete(uint16_t usIndex) {
  ...
  if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
    ...
    if (xRet == ESP_OK && usIndex < xRegistry.usNumNetworks) {
      snprintf(cWifiKey, MAX_WIFI_KEY_WIDTH, "%d",
               xRegistry.usStorageIdx[usIndex]);
...
WIFIReturnCode_t WIFI_GetIPInfo(WIFIIPConfiguration_t *pxIPInfo) {
  ...
  if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
    ...
    if (ret == ESP_OK) {
      ...
      configPRINTF(("%s: esp_netif_get_ip_info_error:  %d", __FUNCTION__, ret));
    }
#endif
    /* Return the semaphore. */
    xSemaphoreGive(xWiFiSem);
...
WIFIReturnCode_t WIFI_StartAP(void) {
  ...
  if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
    ...
    // Wait for wifi started event
    xEventGroupWaitBits(wifi_event_group, AP_STARTED_BIT, pdFALSE, pdFALSE,
                        portMAX_DELAY);
    wifi_ret = eWiFiSuccess;
...
WIFIReturnCode_t WIFI_StopAP(void) {
  ...
  if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
    ...
    if (ret == ESP_OK) {
      ...
      xEventGroupWaitBits(wifi_event_group, AP_STOPPED_BIT, pdTRUE, pdFALSE,
                          portMAX_DELAY);
      xRetVal = eWiFiSuccess;
    }
    /* Return the semaphore. */
...
WIFI_ConfigureAP(const WIFINetworkParams_t *const pxNetworkParams) {
  ...
  if (xSemaphoreTake(xWiFiSem, xSemaphoreWaitTicks) == pdTRUE) {
     ...
     */
    memcpy((char *)&wifi_config.ap.ssid, pxNetworkParams->ucSSID,
    ...
    ret =
        WIFI_SetSecurity(pxNetworkParams->xSecurity, &wifi_config.ap.authmode);
    if (ret != ESP_OK) {
